...
\frame{\frametitle{BFS Problem Description}
    \begin{itemize}
        \item Given
            \begin{itemize}
                \item Graph $G=(\V,\E)$
                \item Distinguished source vertex $s \in \V$
            \end{itemize}
        \item Calculate
            \begin{itemize}
...
\framedgraphic{BFS Example}{bfs-1.png}{0.6}

\frame{\frametitle{Terms Used in this Presentation}
    \begin{itemize}
...
\frame{\frametitle{Model of Computation}
    \begin{itemize}
        \item Large shared memory
        \item Consistent caches between cores
        \item Synchronizing $x$ tasks takes $T_\infty=\Th{\log x}$ time
...
\frame{\frametitle{Existing Approaches for PBFS}
    \begin{itemize}
        \item Assumes somewhat unrealistic PRAM model \maybepause
        ...
        \item Specialized for sparse (dense) graphs only
        ...
        \item Offloads some work to scheduler
            \begin{itemize}
                \item Work-stealing (randomized) gives at best high probability bounds
                    \cite{mit-bag}
            \end{itemize}
...
\frame{\frametitle{Bottlenecks for Parallelizing BFS}
    \begin{itemize}
        \item FIFO
        \item $Dist$ array
    \end{itemize}
...
\frame{\frametitle{Clean BFS - Approach}
    \begin{itemize}
        \item Evenly split edges among cores using prefix sum and binary search
        \item Combine output queues using prefix sum
    \end{itemize}
    %TODO improve this slide
}

\subsection{Algorithm}
\frame{\frametitle{CBFS - Prepare to Split Work}
    \begin{itemize}
        \item One input queue $Q_{in}\subseteq \V$.
            \begin{itemize}
                \item Each vertex in $Q_{in}$ is unique*
                \item $\forall_{u\in Q_{in}} Dist_u = \ell$
                \item $\forall_{u\in Q_{in}} \size{\Gamma_u} > 0$
            \end{itemize}
...
\frame{\frametitle{CBFS - Split Work and Process Edges}
    \begin{itemize}
        \item Each core $i$ processes $m_\ell/P$ edges
            \begin{itemize}
                \item searches $OutDegrees$ for $1+\floor{\frac{i \cdot m_\ell}{P}}$ to find starting edge
...
\frame{\frametitle{CBFS - Dedup Vertexes and Combine Queues}
    \begin{itemize}
        \item $Size_{-1} = 0$
...
\frame{\frametitle{CBFS - Reducing $W_P$ for binary searches}
    \begin{itemize}
        ...
        \item Each core $i$
            \begin{itemize}
                ...
                \item $FirstCore \gets \ceil{\frac{P\cdot FirstDegree}{m_\ell}}$
                \item $LastCore \gets \ceil{\frac{P\cdot FirstDegreeNext}{m_\ell}}$
...
\frame{\frametitle{CBFS - Reducing $W_P$ for parallel prefix sums}
    \begin{itemize}
...
\frame{\frametitle{Future Work}
    \begin{itemize}
        \item Optimize CBFS for the PRAM model
            \begin{itemize}
                \item CBFS runs in same time for PRAM but is not asymptotically optimal
            \end{itemize}
        \item Modify CBFS to remove false sharing
    \end{itemize}
}
